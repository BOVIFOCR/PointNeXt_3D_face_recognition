        ...
        with open(protocol_file_path, 'r') as fp:
            ...
            while global_pair_idx < total_num_pairs:
                ...
                for _ in range(fold_size):
                    pos_pair = all_lines[global_pair_idx].split('\t')   # Carl_Reiner_0001_0000.jpg	Carl_Reiner_0003_0000.jpg	1
                    subj1, subj2, pair_label = pos_pair[0].split('.')[0], pos_pair[1].split('.')[0], pos_pair[2]
                    assert pair_label == pos_pair_label
                    
                    pattern_path = os.path.join(dataset_path, subj1, '*'+file_ext)
                    path_sample1 = glob(pattern_path)
                    if len(path_sample1) > 0:
                        path_sample1 = path_sample1[0]
                    else:
                        print('Error, not file found: \'' + pattern_path + '\'')
                        print(f'global_pair_idx: {global_pair_idx}/{total_num_pairs} -', pos_pair)
                        sys.exit(0)
                    
                    pattern_path = os.path.join(dataset_path, subj2, '*'+file_ext)
                    path_sample2 = glob(pattern_path)
                    if len(path_sample2) > 0:
                    ...
                    pos_pair = (pair_label, path_sample1, path_sample2)
                ...
                neg_pairs_paths = []
                for _ in range(fold_size):
                    neg_pair = all_lines[global_pair_idx].split('\t')   # Jeffrey_Archer_0002_0000.jpg	Luis_Ernesto_Derbez_Bautista_0003_0001.jpg	0
                    subj1, subj2, pair_label = neg_pair[0].split('.')[0], neg_pair[1].split('.')[0], neg_pair[2]
                    assert pair_label == neg_pair_label
                    ...
                    if len(path_sample1) > 0:
                        ...
                        print('Error, not file found: \'' + pattern_path + '\'')
                        print(f'global_pair_idx: {global_pair_idx}/{total_num_pairs} -', neg_pair)
                        sys.exit(0)
                    ...
                    neg_pair = (pair_label, path_sample1, path_sample2)
