...
class VerificationTester:
    ...
    def __init__(self):
        ...
        self.LFW_VERIF_PAIRS_LIST = '/datasets1/bjgbiesseck/lfw/pairs.txt'
    ...
    def do_fusion_verification_test(self, model, dataset='LFW', num_points=1200, distance_pairs_2d=np.array([]), verbose=True):
        model.eval()
        ...
        with torch.no_grad():
            distances_3d = torch.zeros(int(len(cache['x'])/2), device='cuda:0')
            batch_size = 64
            ...
            for i in range(0, num_batches):
                ...
                distances_3d[int(start_batch_idx/2):int(end_batch_idx/2)] = dist

                if verbose:
                    print('---------------')

            fused_distances = self.fuse_scores(distances_3d, distance_pairs_2d)

            # print('fused_distances:', fused_distances)
            # print('fused_distances.size():', fused_distances.size())
            if verbose:
                print('Findind best treshold...')

            best_tresh, best_acc, tar, far = self.find_best_treshold(dataset, fused_distances, verbose=verbose)
            return best_tresh, best_acc, tar, far
...
if __name__ == "__main__":
    ...
    # Load 2D distances
    distance_pairs_2d = np.load(args.arcdists)
    distance_pairs_2d = torch.from_numpy(distance_pairs_2d).float().to(0)
    # print('distance_pairs_2d:', distance_pairs_2d.device)
    # sys.exit(0)

    # Do fused verification test
    best_tresh, best_acc, tar, far = verif_tester.do_fusion_verification_test(model, args.dataset, args.num_points, distance_pairs_2d, verbose=True)
