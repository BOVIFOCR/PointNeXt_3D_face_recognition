...
from .tree_casia_webface_3Dreconstructed_MICA import TreeCASIA_WEBFACE_3DReconstructedMICA


@DATASETS.register_module()
class CASIA_WEBFACE_3D_MICA(Dataset):
    ...
    def __init__(self,
        ...
        if not os.path.isfile(path_paths_file):
            subjects_with_pc_paths, unique_subjects_names, samples_per_subject = TreeCASIA_WEBFACE_3DReconstructedMICA().load_filter_organize_pointclouds_paths(self.DATA_PATH, dir_level, file_ext, min_samples, max_samples)
            ...
            with open(path_paths_file, 'wb') as file:
                print('CASIA_WEBFACE_3D_MICA.__init__ - Saving point cloud paths:', path_paths_file)
                pickle.dump(paths_dict, file)

        else:
            with open(path_paths_file, 'rb') as file:
                print('CASIA_WEBFACE_3D_MICA.__init__ - Reading point cloud paths:', path_paths_file)
