...
if __name__ == '__main__':
    ...
    # all_pc_paths, all_pc_subjects = TreeWEBFACE_3DReconstructedMICA().get_all_pointclouds_paths(dir_path=dataset_path, dir_level=dir_level, pc_ext=file_ext)
    # unique_subjects_names, samples_per_subject = TreeWEBFACE_3DReconstructedMICA().count_samples_per_subject(all_pc_paths, pc_ext=file_ext)
    ...
    print('Searching all files ending with \'' + file_ext + '\' in \'' + dataset_path + '\' ...')
    all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, indexes_samples = TreeWEBFACE_3DReconstructedMICA().get_all_pointclouds_paths_count(dataset_path, dir_level, file_ext)
    # for pc_path, pc_subject in zip(all_pc_paths, all_pc_subjects):
    #     print('pc_path:', pc_path, '    pc_subject:', pc_subject)
    # print('len(all_pc_paths):', len(all_pc_paths), '    len(all_pc_subjects):', len(all_pc_subjects))
    # raw_input('Paused, press ENTER')
    ...
    # pos_pairs, neg_pairs = TreeWEBFACE_3DReconstructedMICA().make_pairs_global_indexes(all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, indexes_samples, num_pos_pairs, num_neg_pairs, reuse_samples)
    # pos_pairs_format_labels_paths, neg_pairs_labels_paths, pos_pair_label, neg_pair_label = TreeWEBFACE_3DReconstructedMICA().make_pairs_labels_with_paths(all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, indexes_samples, num_pos_pairs, num_neg_pairs, reuse_samples)

    print('Making train and test pairs to save in file...')
    pos_pairs_format_lfw, neg_pairs_format_lfw = TreeWEBFACE_3DReconstructedMICA().make_pairs_indexes_lfw_format(all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, indexes_samples, num_pos_pairs, num_neg_pairs, reuse_samples)
    perc_train = 0.8
    ...
    TreeWEBFACE_3DReconstructedMICA().save_pairs_txt_file_lfw_format(pos_pairs_format_lfw, neg_pairs_format_lfw, perc_train, perc_test, pairs_file_path)
    print('Saved!')
